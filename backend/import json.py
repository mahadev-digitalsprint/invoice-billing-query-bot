...
def _coerce_project_billing_summary(value: Any) -> dict[str, Any]:
    summary = value if isinstance(value, dict) else {}
    return {
        "total_invoiced": _normalize_whitespace(summary.get("total_invoiced")),
        "total_paid": _normalize_whitespace(summary.get("total_paid")),
        "outstanding": _normalize_whitespace(summary.get("outstanding")),
    }
...
def _coerce_project_invoices(items: Any) -> list[dict[str, Any]]:
    if not isinstance(items, list):
        return []
    ...
    for item in items:
        ...
        normalized.append(
            {
                "invoice_number": _normalize_whitespace(item.get("invoice_number")),
                "invoice_date": _normalize_whitespace(item.get("invoice_date")),
                "billing_period": _normalize_whitespace(item.get("billing_period")),
                "amount": _normalize_whitespace(item.get("amount")),
                "description": _normalize_whitespace(item.get("description")),
                "payment_terms": _normalize_whitespace(item.get("payment_terms")),
                "status": _normalize_whitespace(item.get("status")),
                "payment_date": _normalize_whitespace(item.get("payment_date")),
                "due_date": _normalize_whitespace(item.get("due_date")),
                "overdue_details": _normalize_whitespace(item.get("overdue_details")),
                "follow_up_actions": _coerce_notes(item.get("follow_up_actions")),
...
def _coerce_projects(items: Any) -> list[dict[str, Any]]:
    if not isinstance(items, list):
        return []
    ...
    for item in items:
        ...
        normalized.append(
            {
                "project_name": _normalize_whitespace(item.get("project_name")),
                "client_name": _normalize_whitespace(item.get("client_name")),
                "invoices": _coerce_project_invoices(item.get("invoices")),
                "billing_summary": _coerce_project_billing_summary(
                    item.get("billing_summary")
                ),
                "observations": _coerce_notes(item.get("observations")),
            }
        )
    return normalized


def _coerce_common_billing_policies(value: Any) -> dict[str, Any]:
    policies = value if isinstance(value, dict) else {}
    return {
        "payment_terms": _normalize_whitespace(policies.get("payment_terms")),
        "overdue_definition": _normalize_whitespace(
            policies.get("overdue_definition")
        ),
        "follow_up_process": _coerce_notes(policies.get("follow_up_process")),
        "dispute_handling": _normalize_whitespace(policies.get("dispute_handling")),
    }


def _coerce_sample_questions(items: Any) -> list[dict[str, Any]]:
    if not isinstance(items, list):
        return []
...
class RAGService:
    ...
    def extract_structured_invoice(self, pdf_path: Path) -> dict[str, Any]:
        ...
        record = {
            ...
            "document_type": _normalize_whitespace(payload.get("document_type")),
            "project_name": _normalize_whitespace(payload.get("project_name")),
            "invoice_number": _normalize_whitespace(payload.get("invoice_number")),
            "issue_date": _normalize_whitespace(payload.get("issue_date")),
            "billing_period": _normalize_whitespace(payload.get("billing_period")),
            "currency": _normalize_whitespace(payload.get("currency")),
            "payment_terms": _normalize_whitespace(payload.get("payment_terms")),
            "status": _normalize_whitespace(payload.get("status")),
            "payment_date": _normalize_whitespace(payload.get("payment_date")),
            "due_date": _normalize_whitespace(payload.get("due_date")),
            "overdue_details": _normalize_whitespace(payload.get("overdue_details")),
            "follow_up_actions": _coerce_notes(payload.get("follow_up_actions")),
            ...
            "projects": _coerce_projects(payload.get("projects")),
            "common_billing_policies": _coerce_common_billing_policies(
                payload.get("common_billing_policies")
            ),
            "sample_questions": _coerce_sample_questions(
                payload.get("sample_questions")
            ),
